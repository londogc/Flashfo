      ...
      <div style={panel}>
        ...
        {JUMP.map((item,i) => (
          <a key={item.label} href={item.href} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 0', borderBottom: i<JUMP.length-1?'1px solid var(--c-line)':'none', textDecoration:'none' }}>
            <div style={{ width:32, height:32, background:item.ib.replace('bg-','').replace('/10','').replace('-500',''), borderRadius:10, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}
              className={item.ib}>
