...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (profile) setForm({ full_name: profile.full_name || '', username: profile.username || '', bio: profile.bio || '' })
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%', color:'var(--c-t3)', fontSize:14 }}>Loading...</div>
  )
  ...
  return (
    <div style={{ padding:'24px 20px', maxWidth:600, margin:'0 auto' }}>
      ...
      <div style={{ background:'var(--c-surface)', border:'1px solid var(--c-line)', borderRadius:18, padding:20, marginBottom:16, display:'flex', alignItems:'center', gap:16 }}>
        ...
        <div>
          <div style={{ fontSize:14, fontWeight:700, color:'var(--c-t1)' }}>{form.full_name || 'Your Name'}</div>
      ...
      <form onSubmit={save} style={{ background:'var(--c-surface)', border:'1px solid var(--c-line)', borderRadius:18, padding:20 }}>
        ...
        {success && <div style={{ marginTop:14, padding:'10px 14px', background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:10, fontSize:13, color:'#16a34a' }}>Profile saved!</div>}

        <div style={{ marginTop:20, display:'flex', gap:10 }}>
          <button type="submit" disabled={saving} style={{ height:40, padding:'0 20px', background:'#1d4ed8', color:'white', border:'none', borderRadius:10, fontSize:13, fontWeight:700, cursor:saving?'not-allowed':'pointer', opacity:saving?0.7:1 }}>
      ...
      <div style={{ background:'var(--c-surface)', border:'1px solid #fecaca', borderRadius:18, padding:20, marginTop:16 }}>
        ...
        <p style={{ fontSize:12, color:'var(--c-t2)', marginBottom:12 }}>You will be signed out of your account on this device.</p>
