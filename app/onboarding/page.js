        ...
        <div style={{ background:'#161b22', border:'1px solid #21262d', borderRadius:16, padding:32 }}>
          ...
          {step === 1 && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
              {['Student','Teacher','Parent','Self-learner'].map(r => (
                <button key={r} onClick={() => setRole(r)}
                  style={{ padding:'16px 12px', borderRadius:10, border:'1px solid', cursor:'pointer', textAlign:'center',
                    borderColor: role===r ? '#a78bfa' : '#30363d',
                    background: role===r ? 'rgba(167,139,250,0.08)' : '#0d1117',
                    color: role===r ? '#a78bfa' : '#8b949e', fontSize:14, fontWeight:500 }}>
                  {r==='Student'?'🎓':r==='Teacher'?'📖':r==='Parent'?'👨‍👩‍👦':'💡'} {r}
                </button>
              ))}
            </div>
          )}
