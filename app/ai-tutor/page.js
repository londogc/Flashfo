...
export default function AITutorPage() {
  const [msgs, setMsgs] = useState([{
    role: 'assistant',
    content: "Hi! I'm your AI Tutor. Ask me any question or paste notes and I'll explain concepts simply and clearly."
  }])
  ...
  async function send() {
    ...
    try {
      ...
      const reply = typeof data.result === 'string'
        ? data.result
        : data.error || "I couldn't process that. Try asking differently."
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-6 py-4 border-b border-line bg-surface flex-shrink-0">
        ...
        <p className="text-xs text-t2 mt-0.5">Ask any question — get clear, simple explanations on any topic.</p>
      </div>
      ...
      <div className="px-4 py-4 border-t border-line bg-surface flex-shrink-0">
        <div className="flex gap-2 items-end max-w-4xl mx-auto">
          <textarea value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="Ask a question or paste notes to explain..."
