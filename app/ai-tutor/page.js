...
export default function AITutorPage(){
  const[msgs,setMsgs]=useState([{role:'assistant',content:'Hi! I\'m your AI Tutor. Paste some notes or ask me anything about a topic you\'re studying.'}])
  ...
  async function send(){
    ...
    try{
      const res=await fetch('/api/rpc',{
        ...
        body:JSON.stringify({action:'aiTutor',message:text,history:msgs.slice(-10)})
      })
      const data=await res.json()
      const reply=data.result||data.response||data.content||data.message||'I couldn\'t process that. Try rephrasing.'
  ...
  return(
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {msgs.map((m,i)=>(
          <div key={i} className={`flex ${m.role==='user'?'justify-end':''}`}>
            {m.role==='assistant'&&<div className="w-7 h-7 rounded-full bg-blue-700 flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0 mr-3 mt-0.5">AI</div>}
      ...
      <div className="px-4 py-4 border-t border-line bg-surface flex-shrink-0">
        ...
        <p className="text-center text-[10px] text-t3 mt-2">Press Enter to send · Shift+Enter for new line</p>
      </div>
    </div>
  )
}