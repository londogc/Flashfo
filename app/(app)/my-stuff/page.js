...
export default function MyStuffPage() {
  ...
  // Mounted guard — prevents auth check firing during SSR / before Supabase resolves
  ...
  // Show loading until mounted AND auth has resolved
  if (!mounted || authLoading) return (
    <div className="p-6 max-w-4xl mx-auto w-full">
      <div className="h-8 w-40 bg-surface2 rounded-xl animate-pulse mb-2"/>
      <div className="space-y-3 mt-6">{[...Array(3)].map((_,i) => <div key={i} className="h-20 bg-surface border border-line rounded-xl animate-pulse"/>)}</div>
    </div>
  )

  if (!user) return (
    <div className="p-6 max-w-4xl mx-auto w-full text-center py-20">
          ...
          {filtered.map(item => {
            ...
            return (
              <div key={item.id} className="bg-surface border border-line rounded-xl overflow-hidden transition-all">
                <div className="flex items-center gap-3 p-4 cursor-pointer hover:bg-surface2 transition-colors"
                  ...
                  <div className="border-t border-line p-4">
                    {item.type === 'quiz' && item.data?.questions && (
                      <div className="space-y-2">
                        {item.data.questions.slice(0,3).map((q,i) => <div key={i} className="text-[12px] text-t2"><span className="font-semibold text-t1">{i+1}.</span> {q.question}</div>)}
                    ...
                    {item.type === 'flashcards' && item.data?.cards && (
                      <div className="space-y-2">
                        {item.data.cards.slice(0,3).map((c,i) => <div key={i} className="text-[12px] text-t2"><span className="font-semibold text-t1">{c.front||c.question}:</span> {c.back||c.answer}</div>)}
                        {item.data.cards.length > 3 && <div className="text-[11px] text-t3">+{item.data.cards.length-3} more cards</div>}
                      </div>
                    )}
                    {['lesson_plan','study_guide','summary'].includes(item.type) && (item.data?.output||item.data?.plan) && (
                      <p className="text-[12px] text-t2 whitespace-pre-wrap line-clamp-6">{(item.data.output||item.data.plan||'').substring(0,400)}{(item.data.output||item.data.plan||'').length>400?'...':''}</p>
                    )}
                    {item.type === 'conversation' && item.data?.messages && (
                      <div className="space-y-2">
                        {item.data.messages.slice(0,4).map((m,i) => <div key={i} className={`text-[12px] ${m.role==='user'?'text-t1':'text-t2'}`}><span className="font-semibold">{m.role==='user'?'You':'Nova'}:</span> {(m.content||'').slice(0,120)}{(m.content||'').length>120?'...':''}</div>)}
