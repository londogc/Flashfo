...
function detectNovaAction(content) {
  ...
  if ((lower.includes('flashcard') || lower.includes('flash card')) &&
    (content.match(/\d+\.\s+.+[:\u2014\-]/) || content.match(/front:|back:/i))) return 'flashcards'
...
function NovaActionChip({ action, content, onAction }) {
  ...
  const cfg = ({
    flashcards: { text: '\u26a1 Study as flashcards', color: '#818cf8', bg: 'rgba(99,102,241,0.15)', border: 'rgba(99,102,241,0.3)' },
    quiz: { text: '\u2753 Take as a quiz', color: '#34d399', bg: 'rgba(16,185,129,0.12)', border: 'rgba(16,185,129,0.28)' },
    study_guide: { text: '\uD83D\uDCCB Open study guide', color: '#60a5fa', bg: 'rgba(37,99,235,0.12)', border: 'rgba(37,99,235,0.28)' },
  })[action] || { text: 'Study', color: '#818cf8', bg: 'rgba(99,102,241,0.15)', border: 'rgba(99,102,241,0.3)' }
  return (
...
function parseBlocks(content) {
  ...
  while ((m = re.exec(content)) !== null) {
    ...
    parts.push({ type: 'code', lang: (m[1] || 'text').toLowerCase(), code: m[2].trim() })
...
function CodeBlock({ lang, code }) {
  ...
  function download() {
    const ext = lang === 'javascript' || lang === 'jsx' ? 'js' : lang === 'text' ? 'txt' : lang
    const blob = new Blob([code], { type: 'text/plain' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `nova-code.${ext}`; a.click()
    URL.revokeObjectURL(url)
  }
  const srcDoc = lang === 'html' ? code
    : lang === 'css' ? `<style>${code}</style><p style="font-family:sans-serif;padding:16px;color:#333">CSS preview</p>`
    : (lang === 'js' || lang === 'javascript' || lang === 'jsx')
      ? `<script>try{${code}}catch(e){document.body.innerHTML='<pre style="color:red">'+e+'</pre>'}<\/script>`
      : code
  return (
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {previewable && (
            <button onClick={() => setPreview(p => !p)}
              style={{ fontSize: 11, color: '#818cf8', background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 6, padding: '3px 9px', cursor: 'pointer' }}>
              {preview ? 'Code' : 'Preview'}
            </button>
          )}
          <button onClick={download}
            style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, padding: '3px 9px', cursor: 'pointer' }}>
            Download
          </button>
        </div>
      </div>
...
export default function NovaPage() {
  ...
  useEffect(() => {
    const shellBar = Array.from(document.querySelectorAll('div')).find(el => {
      const s = el.getAttribute('style') || ''
      return s.includes('height: 52px') && s.includes('flex-shrink: 0')
    })
    if (shellBar) { shellBar.style.display = 'none'; shellBar._novaHid = true }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (msgs?.length) { setMessages(msgs); setGreeting(false); setHandoffBanner(true); setTimeout(() => setHandoffBanner(false), 4000) }
    } catch (e) { }
  }, [])
  ...
  const scrollInputIntoView = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const overlap = barRect.bottom - visibleBottom
      if (overlap > 0 && msgsRef.current) msgsRef.current.scrollTop += overlap + 16
    }, 100)
  }, [])
  ...
  const handleNovaAction = useCallback(async (action, sourceContent) => {
    try {
      if (action === 'flashcards') {
        const data = await rpc('generateFlashcardsFromText', [sourceContent, 10, 'English'])
        const cards = data?.result?.cards || []
        if (cards.length) { sessionStorage.setItem('flashfo_load_flashcards', JSON.stringify({ cards, topic: 'Nova deck' })); window.open('/flashcards', '_blank') }
      } else if (action === 'quiz') {
        const data = await rpc('generateQuizAdvancedFromText', [sourceContent, { mcq: 5, trueFalse: 2, shortAnswer: 0, difficulty: 'medium' }, 'English'])
        const questions = data?.result?.questions || []
        if (questions.length) { sessionStorage.setItem('flashfo_quiz_load', JSON.stringify({ questions, topic: 'Nova quiz' })); window.open('/quiz', '_blank') }
      } else if (action === 'study_guide') {
        const data = await rpc('generateStudyGuideFromText', [sourceContent, 'English'])
        if (data?.result) { sessionStorage.setItem('flashfo_studyguide_load', JSON.stringify({ guide: data.result, topic: 'Nova study guide' })); window.open('/study-guide', '_blank') }
      }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', maxHeight: '100dvh', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, zIndex: 2, position: 'relative', padding: '10px 14px 8px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.07)', border: '0.5px solid rgba(129,140,248,0.32)', borderRadius: 30, padding: '7px 10px', display: 'flex', alignItems: 'center', gap: 8, backdropFilter: 'blur(12px)' }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.78)', flex: 1 }}>
            Nova · {stateLabel}
          </span>
          {handoffBanner && (
      ...
      <div ref={msgsRef} className="nv-msgs" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch', padding: '12px 16px 8px', display: 'flex', flexDirection: 'column', gap: 12, position: 'relative', zIndex: 1 }}>
        {greeting && (
          <div style={{ textAlign: 'center', padding: '20px 16px 12px', flexShrink: 0 }}>
            <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-.04em', marginBottom: 8, background: 'linear-gradient(135deg,#fff,#a5b4fc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              What are we working on?
            </div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,.35)', lineHeight: 1.6 }}>
              Ask me anything — I’ll explain, write code, debug errors, or build flashcards.
            </div>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginTop: 16 }}>
              {['Explain a concept', 'Debug my code', 'Write a quiz', 'Summarize notes'].map(q => (
                <button key={q} onClick={() => send(q)}
                  style={{ fontSize: 12, color: 'rgba(167,139,250,0.75)', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.22)', borderRadius: 20, padding: '6px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {q}
                </button>
              ))}
            </div>
              ...
              <div style={{ maxWidth: '84%' }}>
                {m.images?.length > 0 && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    {m.images.map((img, ii) => (
                      <img key={ii} src={img.preview} alt="Attached" style={{ maxWidth: 140, maxHeight: 100, borderRadius: 8, objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)' }} />
                    ))}
                  </div>
                )}
                {m.content && (
                  <div style={{ padding: '11px 14px', borderRadius: m.role === 'user' ? '18px 18px 5px 18px' : '18px 18px 18px 5px', background: m.role === 'user' ? 'linear-gradient(135deg,rgba(79,70,229,.28),rgba(109,40,217,.22))' : 'rgba(8,12,22,.88)', border: `1px solid ${m.role === 'user' ? 'rgba(99,102,241,.3)' : 'rgba(255,255,255,.09)'}`, color: 'rgba(255,255,255,.88)', fontSize: 14, lineHeight: 1.65, backdropFilter: 'blur(12px)' }}>
        ...
        {loading && novaState === 'thinking' && (
          <div style={{ display: 'flex' }}>
            <div style={{ padding: '12px 16px', borderRadius: '18px 18px 18px 5px', background: 'rgba(8,12,22,.88)', border: '1px solid rgba(255,255,255,.09)', display: 'flex', gap: 5, alignItems: 'center', backdropFilter: 'blur(12px)' }}>
              {[0, 150, 300].map((d, j) => (
                <div key={j} style={{ width: 7, height: 7, borderRadius: '50%', background: 'rgba(129,140,248,.7)', animation: `nv-bounce .9s ease-in-out ${d}ms infinite` }} />
              ))}
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: 8, paddingBottom: 8, flexWrap: 'wrap' }}>
            {pendingImages.map((img, i) => (
              <div key={i} style={{ position: 'relative' }}>
                ...
                <button onClick={() => setPendingImages(prev => prev.filter((_, ii) => ii !== i))}
                  style={{ position: 'absolute', top: -5, right: -5, width: 17, height: 17, borderRadius: '50%', background: '#ef4444', border: '1.5px solid #07090f', cursor: 'pointer', color: '#fff', fontSize: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, padding: 0 }}>
                  ×
                </button>
              </div>
            ))}
