...
function printQuizBlank(questions, topic) {
  ...
  const qHtml = questions.map(function(q, i) {
    ...
    else if (q.type === 'matching') { body = (q.pairs||[]).map(function(p,j){ return '<div style="display:flex;gap:16px;margin:4px 0"><div style="flex:1;padding:4px 8px;border:1px solid #ddd;border-radius:4px">'+(j+1)+'. '+p.left+'</div><div style="flex:1;padding:4px 8px;border:1px solid #ddd;border-radius:4px">___________</div></div>' }).join('') }
    else { body = (q.options||(q.type==='true_false'?['True','False']:[])).map(function(o,j){ return '<div style="padding:4px 8px;border:1px solid #ddd;border-radius:4px;margin:3px 0;font-size:12px">'+labels[j]+'. '+o+'</div>' }).join('') }
...
function printQuizKey(questions, topic) {
  ...
  const qHtml = questions.map(function(q, i) {
    ...
    else if (q.type === 'matching') { body = (q.pairs||[]).map(function(p,j){ return '<div style="font-size:12px;margin:2px 0"><strong>'+(j+1)+'. '+p.left+'</strong> &rarr; '+p.right+'</div>' }).join('') }
    else { body = (q.options||(q.type==='true_false'?['True','False']:[])).map(function(o,j){ var correct = j === q.answerIndex; return '<div style="padding:4px 8px;border:1px solid '+(correct?'#6ee7b7':'#e5e7eb')+';background:'+(correct?'#d1fae5':'transparent')+';border-radius:4px;margin:3px 0;font-size:12px;font-weight:'+(correct?'600':'normal')+'">'+labels[j]+'. '+o+(correct?' &#10003;':'')+'</div>' }).join('') }
...
function SpeakerBtn({ text }) {
  ...
  return (
    <button onClick={e=>{e.stopPropagation();speak()}} title="Listen" className="flex-shrink-0 flex items-center justify-center w-7 h-7 rounded-full hover:bg-blue-500/10 transition-colors" style={{color:busy?'#93c5fd':'#60a5fa',opacity:busy?0.6:1}}>
      ...
      <div className="bg-surface border border-line rounded-2xl w-full max-w-2xl shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-line">
          ...
          <div className="flex gap-2">
            <button onClick={()=>printQuizKey(questions,topic)} className="h-8 px-3 bg-blue-700 text-white text-[12px] font-semibold rounded-lg hover:bg-blue-800 flex items-center gap-1.5">
                  ...
                  {q.type==='mcq'&&selected&&selected[i]!==undefined&&selected[i]!==q.answerIndex&&(
                    <div style={{marginTop:12}}>
                      {novaExplanations&&novaExplanations[i]?(
                        ...
                        <button onClick={()=>explainWrongAnswer&&explainWrongAnswer(i,q,q.options?.[selected[i]]||'Your answer',q.options?.[q.answerIndex]||'Correct answer')} disabled={explanationLoading&&explanationLoading[i]} style={{display:'flex',alignItems:'center',gap:7,padding:'8px 14px',background:'rgba(167,139,250,.07)',border:'1px solid rgba(167,139,250,.18)',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:600,color:'#a78bfa'}}>
    ...
    <div className="p-6 max-w-3xl mx-auto w-full">
        ...
        {qs.map((q,i)=>(
          <div key={i} className="bg-surface border border-line rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 uppercase">{(q.type||'mcq').replace(/_/g,' ')}</span>
            ...
            <textarea value={q.question} onChange={e=>updateQ(i,'question',e.target.value)} className="w-full text-sm text-t1 bg-surface2 border border-line rounded-lg p-2 resize-none outline-none focus:border-blue-400 mb-2" rows={2}/>
            {(q.type==='short_answer'||q.type==='fill_blank')&&<input value={q.correctAnswer||''} onChange={e=>updateQ(i,'correctAnswer',e.target.value)} placeholder="Correct answer..." className="w-full h-8 text-[13px] text-t1 bg-surface2 border border-line rounded-lg px-2 outline-none focus:border-blue-400"/>}
            {(q.type==='mcq'||q.type==='true_false'||!q.type)&&(
              <div className="space-y-1.5">{(q.options||['True','False']).map((o,j)=>(
                <div key={j} className="flex items-center gap-2">
                  <button onClick={()=>updateQ(i,'answerIndex',j)} className={'w-6 h-6 rounded-full border-2 flex items-center justify-center text-[10px] flex-shrink-0 transition-colors '+(q.answerIndex===j?'border-emerald-500 bg-emerald-500 text-white':'border-line text-t3 hover:border-emerald-400')}>{q.answerIndex===j?'✓':['A','B','C','D'][j]}</button>
                  {q.type==='true_false'?<span className="flex-1 text-[13px] text-t1 px-2">{o}</span>:<input value={o} onChange={e=>updateOpt(i,j,e.target.value)} className="flex-1 h-8 text-[13px] text-t1 bg-surface2 border border-line rounded-lg px-2 outline-none focus:border-blue-400"/>}
                </div>
              ))}</div>
            )}
          </div>
      ...
      {!addType?(
        <div className="border-2 border-dashed border-line rounded-xl p-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {['mcq','true_false','short_answer','fill_blank','matching'].map(t=>(
              <button key={t} onClick={()=>setAddType(t)} className="h-8 px-3 bg-surface2 border border-line text-t2 text-[12px] font-medium rounded-lg hover:border-blue-400 hover:text-blue-500 transition-colors">
        ...
        <div className="border-2 border-blue-300/40 rounded-xl p-4 bg-blue-500/5">
          ...
          <textarea value={newQ.question} onChange={e=>setNewQ(q=>({...q,question:e.target.value}))} placeholder="Question text..." rows={2} className="w-full text-sm text-t1 bg-surface border border-line rounded-lg p-2 resize-none outline-none focus:border-blue-400 mb-3"/>
          {(addType==='short_answer'||addType==='fill_blank')&&<input value={newQ.correctAnswer} onChange={e=>setNewQ(q=>({...q,correctAnswer:e.target.value}))} placeholder="Correct answer..." className="w-full h-8 text-[13px] text-t1 bg-surface border border-line rounded-lg px-2 outline-none focus:border-blue-400 mb-3"/>}
          {addType==='mcq'&&<div className="space-y-1.5 mb-3">{newQ.options.map((o,j)=>(
            <div key={j} className="flex items-center gap-2">
              <button onClick={()=>setNewQ(q=>({...q,answerIndex:j}))} className={'w-6 h-6 rounded-full border-2 flex items-center justify-center text-[10px] flex-shrink-0 '+(newQ.answerIndex===j?'border-emerald-500 bg-emerald-500 text-white':'border-line text-t3')}>{newQ.answerIndex===j?'✓':['A','B','C','D'][j]}</button>
              <input value={o} onChange={e=>setNewQ(q=>({...q,options:q.options.map((op,oi)=>oi===j?e.target.value:op)}))} placeholder={'Option '+['A','B','C','D'][j]} className="flex-1 h-8 text-[13px] text-t1 bg-surface border border-line rounded-lg px-2 outline-none focus:border-blue-400"/>
            </div>
          ))}</div>}
          {addType==='true_false'&&<div className="flex gap-3 mb-3">{['True','False'].map((o,j)=><button key={j} onClick={()=>setNewQ(q=>({...q,answerIndex:j}))} className={'flex-1 h-9 rounded-lg border text-[13px] font-medium transition-colors '+(newQ.answerIndex===j?'bg-emerald-500 text-white border-emerald-500':'bg-surface border-line text-t2 hover:border-emerald-400')}>{o}</button>)}</div>}
          <button onClick={commitAdd} disabled={!newQ.question.trim()} className="h-8 px-4 bg-blue-700 text-white text-[12px] font-semibold rounded-lg hover:bg-blue-800 disabled:opacity-40">Add Question</button>
        </div>
      )}
    </div>
  )
...
const BASE_TYPES = [
  {id:'mcq',label:'Multiple Choice'},{id:'true_false',label:'True / False'},
  {id:'short_answer',label:'Short Answer'},{id:'fill_blank',label:'Fill in the Blank'},
  {id:'matching',label:'Matching'},{id:'mixed',label:'Mixed'},
]
...
export default function QuizPage() {
  ...
  useEffect(() => {
    const id='quiz-anim'; if(document.getElementById(id))return
    const s=document.createElement('style'); s.id=id
    s.textContent="@keyframes spin{to{transform:rotate(360deg)}} @keyframes progress{0%{width:0%}100%{width:88%}} @keyframes nova-pop{0%{opacity:0;transform:translateY(14px) scale(0.97)}60%{opacity:1;transform:translateY(-3px) scale(1.005)}100%{opacity:1;transform:translateY(0) scale(1)}} @keyframes nova-pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.4;transform:scale(.85)}} .nova-card{opacity:0;animation:nova-pop .42s cubic-bezier(.22,.68,0,1.2) forwards} .nova-dot-pulse{animation:nova-pulse .9s ease-in-out infinite}"
  ...
  useEffect(() => {
    ...
    loadDraft('quiz').then(draft => {
      if (draft?.data?.questions?.length) {
        ...
        setTypeId(draft.data.typeId || 'mcq')
        setCount(draft.data.count || 5)
        setQuestions(draft.data.questions)
        initMatching(draft.data.questions)
        setDraftBanner(true)
      }
    })
  }, [])
  ...
  function initMatching(qs) {
    const s={}
    qs.forEach((q,i)=>{ if(q.type==='matching'&&q.pairs) s[i]=[...q.pairs.map(p=>p.right)].sort(()=>Math.random()-0.5) })
    setShuffledRights(s)
  }
  ...
  function startFresh() {
    setQuestions([]); setError(''); setSavedId(null); setDraftBanner(false)
    clearDraft('quiz')
  }

  const autoScore = submitted ? questions.filter((q,i)=>{ if(q.type==='short_answer'||q.type==='matching')return false; if(q.type==='fill_blank')return checkFitbAnswer(fitbInputs[i],q.correctAnswer); return selected[i]===q.answerIndex }).length : 0
  ...
  if (editMode) return <EditPanel questions={questions} onSave={qs=>{setQuestions(qs);setEditMode(false);setSelected({});setSaInputs({});setFitbInputs({});setMatchAnswers({});setSaGrades({});setSubmitted(false);initMatching(qs)}} onCancel={()=>setEditMode(false)}/>
    ...
    <div className="p-6 max-w-3xl mx-auto w-full">
      ...
      {draftBanner && (
        <div style={{background:'rgba(99,102,241,0.08)',border:'1px solid rgba(99,102,241,0.22)',borderRadius:10,padding:'10px 14px',marginBottom:16,display:'flex',alignItems:'center',gap:10}}>
          ...
          <span style={{fontSize:12,color:'rgba(241,240,255,0.7)',flex:1}}>Resuming your last quiz — <strong style={{color:'rgba(241,240,255,0.9)'}}>{topic}</strong></span>
      ...
      {showSave && (
        <div className="fixed inset-0 z-40 flex items-center justify-center" style={{background:'rgba(0,0,0,0.4)'}}>
          <div className="bg-surface border border-line rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <input value={saveTitle} onChange={e=>setSaveTitle(e.target.value)} placeholder={topic||'Quiz title...'} className="w-full h-9 bg-surface2 border border-line rounded-lg px-3 text-sm text-t1 outline-none focus:border-blue-400 mb-4"/>
            <div className="flex gap-2">
      ...
      {!questions.length ? (
        <div className="bg-surface border border-line rounded-2xl p-5">
          <textarea value={topic} onChange={e=>setTopic(e.target.value)} placeholder="Enter a topic or paste notes to generate a quiz from..." className="w-full h-28 text-sm text-t1 bg-transparent resize-none outline-none placeholder:text-t3 mb-5"/>
          <div className="mb-5">
                  ...
                  return (
                    <div key={k} style={{display:'flex',alignItems:'center',justifyContent:'space-between',background:active?'rgba(37,99,235,0.07)':'var(--c-surface)',border:'1px solid '+(active?'rgba(59,130,246,0.3)':'var(--c-line)'),borderRadius:12,padding:'0 8px 0 16px',height:56,transition:'all 0.15s'}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:2,flexShrink:0}}>
                        <button onClick={()=>setBreakdown(b=>({...b,[k]:Math.max(0,(b[k]||0)-1)}))} style={{width:44,height:44,borderRadius:10,border:'none',background:'none',color:active?'#3b82f6':'var(--c-t3)',fontSize:22,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:300,lineHeight:1}}>&#8722;</button>
                        ...
                        <button onClick={()=>setBreakdown(b=>({...b,[k]:(b[k]||0)+1}))} style={{width:44,height:44,borderRadius:10,border:'none',background:'none',color:active?'#3b82f6':'var(--c-t3)',fontSize:22,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:300,lineHeight:1}}>+</button>
                      </div>
                    </div>
                  )
                })}
          ...
          <div className="mb-5">
            ...
            {typeId!=='mixed'&&(
              <>
                <input type="range" min={5} max={35} step={1} value={count} onChange={e=>setCount(Number(e.target.value))} onInput={e=>setCount(Number(e.target.value))} className="w-full accent-blue-600 cursor-pointer" style={{height:4}}/>
          ...
          <button onClick={generate} disabled={loading||!topic.trim()} style={{width:'100%',padding:'13px 0',borderRadius:10,border:'none',background:loading?'linear-gradient(90deg,#1e3a8a,#4c1d95)':'linear-gradient(90deg,#2563eb,#7c3aed)',color:'#fff',fontSize:14,fontWeight:700,cursor:loading?'not-allowed':'pointer',letterSpacing:'-0.01em',transition:'background 0.3s',position:'relative',overflow:'hidden',opacity:(loading||!topic.trim())?0.6:1}}>
        ...
        <div>
          ...
          <div className="space-y-4 mb-6">
            {questions.map((q,i)=>{
              const isSA=q.type==='short_answer'; const isFITB=q.type==='fill_blank'; const isMatch=q.type==='matching'
              return (
                <div key={i} className="bg-surface border border-line rounded-xl p-4">
                  ...
                  {isFITB&&(
                    <div>
                      <div className="flex items-center gap-2 text-sm text-t1 mb-2">
                        <span>Answer:</span>
                        <input value={fitbInputs[i]||''} onChange={e=>setFitbInputs(s=>({...s,[i]:e.target.value}))} disabled={submitted} placeholder="Fill in the blank..." className="flex-1 h-9 bg-surface2 border border-line rounded-lg px-3 text-sm text-t1 outline-none focus:border-blue-400 disabled:opacity-70"/>
                      </div>
                      {submitted&&<div className={'text-[12px] px-3 py-2 rounded-lg '+(checkFitbAnswer(fitbInputs[i],q.correctAnswer)?'bg-emerald-500/10 text-emerald-600 font-semibold':'bg-red-500/10 text-red-500')}>{checkFitbAnswer(fitbInputs[i],q.correctAnswer)?'✓ Correct!':'✗ Answer: '+q.correctAnswer}</div>}
                    </div>
                  )}
                  {isMatch&&(
                    <div className="space-y-2">
                      ...
                      {(q.pairs||[]).map((pair,j)=>(
                        <div key={j} className="grid grid-cols-2 gap-2 items-center">
                          ...
                          <select value={matchAnswers[i]?.[j]||''} onChange={e=>setMatchAnswers(s=>({...s,[i]:{...(s[i]||{}),[j]:e.target.value}}))} disabled={submitted} className="h-9 bg-surface2 border border-line rounded-lg px-2 text-[13px] text-t1 outline-none focus:border-blue-400 disabled:opacity-70" style={{borderColor:submitted?(matchAnswers[i]?.[j]===pair.right?'#10b981':'#ef4444'):'var(--c-line)'}}>
                  ...
                  {isSA&&(
                    <div>
                      <textarea value={saInputs[i]||''} onChange={e=>setSaInputs(s=>({...s,[i]:e.target.value}))} placeholder="Type your answer here..." disabled={submitted} rows={3} className="w-full text-sm text-t1 bg-surface2 border border-line rounded-lg p-3 resize-none outline-none focus:border-blue-400 disabled:opacity-70 mb-2"/>
                      {submitted&&(
                        <div className="space-y-2">
                          <div className="text-[12px] text-t2 bg-blue-500/10 px-3 py-2 rounded-lg border border-blue-300/30"><span className="font-semibold text-blue-600">Model answer: </span>{q.correctAnswer||'Open-ended'}</div>
                          <div className="flex gap-2 items-center">
                            <span className="text-[11px] text-t3">Self-grade:</span>
                            {['correct','wrong'].map(g=>(
                              <button key={g} onClick={()=>setSaGrades(s=>({...s,[i]:g}))} className={'h-7 px-3 rounded-lg text-[12px] font-medium border transition-colors '+(saGrades[i]===g?(g==='correct'?'bg-emerald-500 text-white border-emerald-500':'bg-red-500 text-white border-red-500'):'border-line text-t2 hover:bg-surface2')}>
                  ...
                  {!isSA&&!isFITB&&!isMatch&&(
                    <div className="space-y-2">
                      {(q.options||['True','False']).map((opt,j)=>{
                        const isSel=selected[i]===j; const isCorr=q.answerIndex===j
                        ...
                        return <button key={j} onClick={()=>!submitted&&setSelected(s=>({...s,[i]:j}))} className={'w-full text-left px-3 py-2.5 rounded-lg border text-[13px] transition-all '+cls}><span className="font-semibold mr-2">{['A','B','C','D'][j]}.</span>{opt}</button>
                      })}
                    </div>
                  )}
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {user&&<button onClick={()=>{setSaveTitle(topic);setShowSave(true)}} className="h-9 px-4 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 flex items-center gap-1.5">&#128190; {savedId?'Update Save':'Save Quiz'}</button>}
            {saveFeedback&&<span className="text-[12px] text-emerald-500 font-medium">{saveFeedback}</span>}
            <button onClick={startFresh} className="h-9 px-4 bg-surface border border-line text-t2 text-sm font-medium rounded-xl hover:bg-surface2 ml-auto">New Quiz</button>
          </div>
        </div>
      )}
    </div>
