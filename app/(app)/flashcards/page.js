    ...
    <div className="p-6 max-w-2xl mx-auto w-full">
      ...
      <div style={{
        ...
        <div style={{fontSize:40,marginBottom:12}}>
          {mastered === cards.length ? '🎉' : hasTrouble ? '📚' : '✅'}
        </div>
        <h2 style={{fontSize:22,fontWeight:900,color:'var(--c-t1)',marginBottom:6,letterSpacing:'-.03em'}}>
          {mastered === cards.length ? 'Perfect session!' : 'Session complete'}
        </h2>
        <p style={{fontSize:13,color:'var(--c-t2)',margin:0}}>
          You studied all {cards.length} cards in <em>{topic}</em>
        </p>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:24}}>
        {[
          { label:'Mastered', value: mastered, color:'#34d399', bg:'rgba(16,185,129,0.08)', border:'rgba(16,185,129,0.2)' },
          { label:'Hard', value: hardCards.length, color:'#fbbf24', bg:'rgba(245,158,11,0.08)', border:'rgba(245,158,11,0.2)' },
          { label:'Again', value: againCards.length, color:'#f87171', bg:'rgba(239,68,68,0.08)', border:'rgba(239,68,68,0.2)' },
        ].map(s => (
        ...
        <div style={{
          ...
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
            <div style={{
              ...
              background:'radial-gradient(circle at 33% 33%,#c4b5fd,#7c3aed 40%,#4c1d95 70%,#08001a)',
              boxShadow:'0 0 12px rgba(124,58,237,0.6)'
            }}/>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:'#a5b4fc'}}>Nova</div>
              <div style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>Study assistant</div>
            </div>
          </div>
          <p style={{fontSize:13,color:'var(--c-t1)',lineHeight:1.6,margin:'0 0 14px'}}>
          ...
          <div style={{marginBottom:14,display:'flex',flexDirection:'column',gap:5}}>
            ...
            {needsWork.length > 4 && (
              <div style={{fontSize:11,color:'var(--c-t3)',padding:'4px 10px'}}>
                + {needsWork.length - 4} more
              </div>
            )}
          </div>
          <button
      ...
      <div style={{display:'flex',gap:10}}>
        <button
          ...
          style={{
            ...
            border:'1px solid var(--c-line)', background:'var(--c-surface2)',
            color:'var(--c-t1)', fontSize:13, fontWeight:700, cursor:'pointer'
          }}>
          ↺ Study again
        </button>
      </div>
...
function FlashcardsPageInner() {
  ...
  function handleHard() {
    ...
    setStudyQueue(q => {
      ...
      const insertAt = Math.max(1, Math.ceil(remaining.length / 2))
      return [
  ...
  function deleteCard(i) {
    ...
    setStudyQueue(q => q.filter(qi => qi !== i).map(qi => qi > i ? qi-1 : qi))
  ...
  async function generate() {
    ...
    setStudyQueue([]); setSessionComplete(false)
  ...
  async function doSave() {
    ...
    try {
      ...
      if (savedId) {
        ...
        setSavedId(r.id); setSaveFeedback('Saved!')
      }
      setShowSave(false); setTimeout(() => setSaveFeedback(''), 3000)
  ...
  function generateFocusedDeck() {
    ...
    const encoded = encodeURIComponent(focusedTopic)
    router.push(`/flashcards?q=${encoded}&autoGenerate=1`)
  }
    ...
    <div className="p-6 max-w-2xl mx-auto w-full">
      {dueToday > 0 && (
        <div style={{ background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.25)', borderRadius:10, padding:'12px 16px', marginBottom:20, display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div style={{ flex:1 }}>
            <p style={{ margin:0, fontWeight:600, fontSize:14, color:'#f59e0b' }}>{dueToday} card{dueToday>1?'s':''} due for review</p>
      ...
      <div className="bg-surface border border-line rounded-2xl p-5">
        ...
        <button onClick={generate} disabled={loading}
          style={{width:'100%',padding:'13px 0',borderRadius:10,border:'none',background:'linear-gradient(90deg,#2563eb,#7c3aed)',color:'#fff',fontSize:14,fontWeight:700,cursor:loading?'not-allowed':'pointer',opacity:loading?0.6:1,letterSpacing:'-0.01em'}}>
          {loading
            ? <span style={{display:'flex',alignItems:'center',justifyContent:'center',gap:9}}>
                <span style={{width:15,height:15,border:'2px solid rgba(255,255,255,0.25)',borderTopColor:'#fff',borderRadius:'50%',flexShrink:0,display:'inline-block',animation:'_fcspin 0.7s linear infinite'}}/>
                Generating...
              </span>
      ...
      {studyQueue.map((qi,pos) => {
        ...
        return (
          <div key={pos} style={{
            ...
            background:color,
            transform:pos===0?'scale(1.5)':'scale(1)',
            transition:'all 0.2s'
          }}/>
        )
      })}
  ...
  const ratingButtons = (
    <div style={{display:'flex',gap:8,marginTop:12,justifyContent:'center'}}>
      <button onClick={handleAgain} style={{flex:1,maxWidth:110,padding:'10px 4px',borderRadius:10,border:'1px solid rgba(239,68,68,0.25)',background:'rgba(239,68,68,0.06)',color:'#f87171',fontSize:11,fontWeight:700,cursor:'pointer'}}>
        <div>Again</div><div style={{fontSize:9,opacity:0.7,marginTop:1}}>→ end</div>
      </button>
      <button onClick={handleHard} style={{flex:1,maxWidth:110,padding:'10px 4px',borderRadius:10,border:'1px solid rgba(245,158,11,0.25)',background:'rgba(245,158,11,0.06)',color:'#fbbf24',fontSize:11,fontWeight:700,cursor:'pointer'}}>
        <div>Hard</div><div style={{fontSize:9,opacity:0.7,marginTop:1}}>→ later</div>
      </button>
      <button onClick={handleEasy} style={{flex:1,maxWidth:110,padding:'10px 4px',borderRadius:10,border:'1px solid rgba(16,185,129,0.3)',background:'rgba(16,185,129,0.08)',color:'#34d399',fontSize:11,fontWeight:700,cursor:'pointer'}}>
        <div>Easy</div><div style={{fontSize:9,opacity:0.7,marginTop:1}}>✓ done</div>
      </button>
    </div>
  )
      ...
      <div className="fc-mobile-wrap p-6 max-w-2xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-t2">{cards.length} cards · {done} done · {remaining} left</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <button onClick={()=>{setCards([]);setStudyQueue([]);setSessionComplete(false);}} className="text-sm text-blue-500 font-medium hover:underline">New</button>
          </div>
        </div>
        ...
        <div style={{display:'flex',gap:4,marginBottom:14,flexWrap:'wrap'}}>
          {studyQueue.map((qi,pos) => {
            const isHard = sessionHardCards.some(c=>(c.front||c.question)===(cards[qi]?.front||cards[qi]?.question))
            const isAgain = sessionAgainCards.some(c=>(c.front||c.question)===(cards[qi]?.front||cards[qi]?.question))
            const col = pos===0?'#3b82f6':isAgain?'#f87171':isHard?'#fbbf24':'rgba(255,255,255,0.15)'
            return <div key={pos} style={{width:8,height:8,borderRadius:'50%',background:col,transform:pos===0?'scale(1.4)':'scale(1)',transition:'all 0.2s'}}/>
          })}
        </div>
        ...
        {flipped ? ratingButtons : (
          <div className="flex gap-3 mt-4 justify-center">
            <p className="text-[12px] text-t3 text-center" style={{padding:'8px 0'}}>Rate this card after revealing the answer</p>
          </div>
        )}
      </div>
      ...
      <div className="fc-desktop-wrap" style={{display:'none'}}>
        ...
        <div style={{padding:'24px 20px',borderRight:'1px solid var(--c-line)',display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:7}}>
            {[
              ...
              {label:'Left',val:remaining,color:'#60a5fa'},
              {label:'Again',val:sessionRatings.again,color:'#f87171'},
              {label:'Hard',val:sessionRatings.hard,color:'#fbbf24'},
            ].map(s=>(
          ...
          <div style={{marginTop:'auto',display:'flex',flexDirection:'column',gap:6}}>
            ...
            <button onClick={()=>{setCards([]);setStudyQueue([]);setSessionComplete(false);}} style={{width:'100%',padding:'6px 0',borderRadius:7,fontSize:11,fontWeight:600,border:'1px solid var(--c-line)',background:'var(--c-surface2)',color:'var(--c-t2)',cursor:'pointer'}}>New Deck</button>
          </div>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'28px 36px'}}>
          ...
          <div style={{position:'relative',width:'100%',maxWidth:440,height:230,marginBottom:22}}>
            ...
            <div onClick={()=>{stopAudio();setFlipped(f=>!f)}}
              style={{position:'absolute',inset:0,background:'var(--c-surface)',border:'1.5px solid '+cardBorderColor,borderRadius:14,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:28,cursor:'pointer',transition:'border-color 0.2s'}}>
