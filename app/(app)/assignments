...
function StudentAssignments({ user }) {
  ...
  const [active, setActive] = useState(null) // currently open assignment
  ...
  async function openAssignment(sub) {
    // Pre-fill answers if in progress
    ...
    if (needsNovaGrade) {
      try {
        ...
        await supabase.from('assignment_submissions')
          ...
          .eq('id', active.id)
      } catch(e) { /* non-critical — Nova grading failed, teacher can still grade manually */ }
    }
    ...
    return (
      <div style={{ maxWidth:680, margin:'0 auto', paddingBottom:40 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
          <button onClick={() => { setActive(null); setAnswers({}) }}
            style={{ ...btn('var(--c-surface2)','var(--c-t2)'), border:'1px solid var(--c-line)', height:32, padding:'0 12px' }}>← Back</button>
          <div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {qs.map((q, i) => (
            <div key={i} style={card}>
              ...
              {q.type === 'short_answer' && (
                <textarea disabled={isSubmitted} value={answers[i] ?? ''}
                  onChange={e => setAnswers(a => ({ ...a, [i]: e.target.value }))}
                  placeholder="Type your answer here…" rows={3}
                  style={{ ...inp, resize:'vertical', opacity:isSubmitted?0.7:1 }}/>
              )}
                  ...
                  {(q.options || ['True','False']).map((opt, oi) => {
                    ...
                    return (
                      <div key={oi} onClick={() => !isSubmitted && setAnswers(a => ({ ...a, [i]: oi }))}
                        style={{ padding:'9px 14px', borderRadius:9, border:`1px solid ${isCorrect?'rgba(52,211,153,0.4)':isWrong?'rgba(239,68,68,0.4)':sel?'rgba(99,102,241,0.4)':'var(--c-line)'}`,
                          background:isCorrect?'rgba(52,211,153,0.06)':isWrong?'rgba(239,68,68,0.06)':sel?'rgba(99,102,241,0.08)':'var(--c-surface2)',
                          cursor:isSubmitted?'default':'pointer', fontSize:13, color:'var(--c-t1)', display:'flex', alignItems:'center', gap:8 }}>
      ...
      {submissions.length === 0 ? (
        ...
        return (
          <div key={sub.id} onClick={() => openAssignment(sub)}
            style={{ ...card, cursor:'pointer', marginBottom:10, display:'flex', alignItems:'center', gap:14,
              borderColor: overdue ? 'rgba(239,68,68,0.3)' : 'var(--c-line)', transition:'border-color 0.15s' }}>
...
function TeacherAssignments({ user }) {
  ...
  // Nova standalone assignment creation
  ...
  async function createNovaAssignment() {
    ...
    try {
      ...
      if (aErr) throw aErr
      setCreateOpen(false); setNewForm({ title:'', instructions:'', topic:'', grade_level:'', grading_mode:'completion', grader:'nova', due_date:'' })
  ...
  if (active) {
    return (
      <div style={{ maxWidth:720, margin:'0 auto', paddingBottom:40 }}>
        ...
        {subLoading ? <div style={{ padding:30, textAlign:'center', color:'var(--c-t3)' }}>Loading submissions…</div> :
        submissions.length === 0 ? <div style={{ ...card, textAlign:'center', padding:36, color:'var(--c-t3)', fontSize:13 }}>No submissions yet.</div> :
        submissions.map(sub => (
                ...
                {active.content?.questions?.map((q, i) => (
                  <div key={i} style={{ padding:'10px 12px', background:'var(--c-surface2)', borderRadius:9, border:'1px solid var(--c-line)' }}>
                    ...
                    <div style={{ fontSize:13, color:'var(--c-t1)', fontWeight:500 }}>
                      {q.type === 'short_answer' ? (sub.answers[i] || '—') :
                       q.options ? q.options[sub.answers[i]] ?? '—' : '—'}
                    </div>
            ...
            {grading === sub.id && (
              <div style={{ marginTop:14, padding:14, background:'rgba(99,102,241,0.06)', borderRadius:10, border:'1px solid rgba(99,102,241,0.2)' }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10, marginBottom:10 }}>
                  <input type="number" min="0" max="100" placeholder="Score /100" value={teacherScore}
                    onChange={e=>setTeacherScore(e.target.value)} style={inp}/>
                  <input placeholder="Feedback note for student" value={teacherNote}
                    onChange={e=>setTeacherNote(e.target.value)} style={inp}/>
                </div>
      ...
      {createOpen && (
        <div style={{ ...card, marginBottom:20, border:'1px solid rgba(99,102,241,0.25)', background:'rgba(99,102,241,0.04)' }}>
          ...
          <div style={{ display:'grid', gap:10, marginBottom:10 }}>
            ...
            <textarea placeholder="Instructions or special notes (optional)" value={newForm.instructions} onChange={e=>setNewForm(f=>({...f,instructions:e.target.value}))} rows={2}
        ...
        return (
          <div key={a.id} onClick={() => openAssignment(a)}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:11, color:'var(--c-t3)' }}>
                {a.nova_generated ? 'Nova generated' : 'Manual'} · {a.content?.questions?.length || 0} questions
