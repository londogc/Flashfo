...
export default function SharedDecksPage() {
  ...
  const loadDecks = useCallback(async () => {
    ...
    setError('')
    try {
      // Fix: removed profiles!user_id join — no FK exists between saved_items.user_id and profiles.id
      // Fetch decks first, then optionally fetch author names separately
      const { data, error: err } = await supabase
        .from('saved_items')
        .select('id, title, data, updated_at, user_id')
        .eq('type', 'flashcards')
        .eq('is_public', true)
      ...
      // Fetch author names separately to avoid FK join issue
      if (deckList.length > 0) {
        const userIds = [...new Set(deckList.map(d => d.user_id).filter(Boolean))]
        ...
        deckList.forEach(d => { d.author_name = profileMap[d.user_id] || 'Anonymous' })
      }

      setDecks(deckList)
    } catch(e) {
      setError('Could not load shared decks. Please try again.')
      console.error('shared-decks error:', e)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div style={{ padding:'28px 24px 56px', maxWidth:900, fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        {user && (
          <a href="/my-stuff" style={{ textDecoration:'none', height:36, padding:'0 16px', borderRadius:10, border:'1px solid rgba(99,102,241,0.3)', background:'rgba(99,102,241,0.08)', color:'#a5b4fc', fontSize:12, fontWeight:700, display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
      ...
      <div style={{ position:'relative', marginBottom:16 }}>
        ...
        <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by topic, subject, or deck name…"
      ...
      {/* Error */}
      {error && (
        <div style={{ fontSize:12, color:'#f87171', marginBottom:16, padding:'8px 12px', background:'rgba(239,68,68,0.07)', borderRadius:8, border:'1px solid rgba(239,68,68,0.2)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          {error}
          <button onClick={() => { setError(''); loadDecks() }} style={{ fontSize:11, color:'#f87171', background:'none', border:'none', cursor:'pointer', textDecoration:'underline' }}>Retry</button>
        </div>
      )}
      ...
      {!loading && !error && filtered.length === 0 && (
        <div style={{ textAlign:'center', padding:'48px 24px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14 }}>
          <div style={{ display:'flex', justifyContent:'center', marginBottom:14 }}>
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1.3" strokeLinecap="round">
          ...
          <div style={{ fontSize:13, color:'var(--c-t2)', lineHeight:1.6 }}>
            {decks.length === 0 ? 'Be the first to share a deck! Go to My Stuff and toggle a deck public.' : `No decks match "${query||subject}". Try a different search term.`}
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:12 }}>
            {filtered.map(deck => {
              ...
              const author     = deck.author_name || 'Anonymous'
              const isOwn      = user?.id === deck.user_id
              const hasCloned  = cloned[deck.id]
              const isCloning  = cloning === deck.id

              return (
                <div key={deck.id} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:14, padding:'18px 18px 14px', display:'flex', flexDirection:'column' }}>
                  <div style={{ flex:1, marginBottom:14 }}>
                    ...
                    {topic !== deck.title && <div style={{ fontSize:12, color:'var(--c-t2)', marginBottom:6, lineHeight:1.4 }}>{topic}</div>}
                    <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                      <span style={{ fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:20, background:'rgba(59,130,246,0.1)', border:'1px solid rgba(59,130,246,0.2)', color:'#60a5fa' }}>{count} card{count!==1?'s':''}</span>
                  ...
                  <div style={{ display:'flex', gap:7 }}>
                    <button onClick={() => { sessionStorage.setItem('flashfo_load_flashcards', JSON.stringify({ cards:deck.data?.cards||[], topic:deck.title })); window.location.href='/flashcards' }}
                    ...
                    {!isOwn && (
                      <button onClick={() => cloneDeck(deck)} disabled={isCloning||hasCloned}
                        ...
                        {isCloning ? '…' : hasCloned ? '✓ Cloned' : 'Clone →'}
                      </button>
                    )}
                    {isOwn && (
