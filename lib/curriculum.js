...
export const CURRICULUM_LIST = Object.keys(US_CURRICULUM)

export function getCurriculumPrompt(curriculum, unit) {
  const c = US_CURRICULUM[curriculum]
  if (!c) return ''
  const unitStr = unit ? ` specifically ${unit},` : ''
  return `This content is aligned to ${c.exam || curriculum} (${c.board || 'US Standards'}).${unitStr} Generate flashcards that test the exact concepts, vocabulary, and skills assessed on this exam. Use US educational terminology and examples relevant to American high school students.`
}